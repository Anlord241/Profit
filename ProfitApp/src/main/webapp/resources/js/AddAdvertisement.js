...
(function () {
  ...
  const form = document.getElementById('add-form');
   ...
   async function upload(file) {
     let toReturn = await fetch(`http://localhost:8080/upload`, { method: 'POST', mode: 'no-cors', body: new FormData(document.forms.add)}) ;
  ...
  function showPicture() {
    ...
    upload(file).then(()=>{
    let photoLink = `C:\\ProfitPhotos\\${file.value.substr(12)}`;
    const photo = document.getElementById('picture');
    if (!file.value){
photoLink = null;
    }
